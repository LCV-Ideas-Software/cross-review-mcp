...
function sliceBeforeStatusBlock(text) {
    ...
    if (!rtrimmed.endsWith(STATUS_CLOSE_TAG)) return { prefix: null, stripped: false };
...
function parseDeclaredModel(text) {
    ...
    if (!stripped) {
        return empty(['status block not at tail; peer-model positioning cannot be validated without a trailing status block']);
    }

    const prefix = rightTrim(rawPrefix);
    if (!prefix.length) {
        return empty(['peer-model block missing (response empty before status block)']);
    }

    if (!prefix.endsWith(MODEL_CLOSE_TAG)) {
        return empty(['peer-model block missing or not positioned immediately before status block']);
    }
    ...
    if (
        ...
        return empty(['peer-model block payload does not contain a string model_id field']);
    }
    ...
    if (id.length > MAX_MODEL_ID_CHARS) {
        return empty([`peer-model block model_id exceeds ${MAX_MODEL_ID_CHARS} chars`]);
    }
