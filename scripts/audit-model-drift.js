...
function ageInDays(isoDate) {
    ...
    if (Number.isNaN(parsed)) {
        fail(3, `STRUCTURAL ERROR: last_verified '${isoDate}' is not ISO-parseable (expected YYYY-MM-DD).`);
    }
...
function validateFallbackChain(agent, entry, errors) {
    ...
    if (!Array.isArray(chain)) {
        errors.push(`entries.${agent}.fallback_chain must be an array (got ${typeof chain})`);
        return;
    }
    if (chain.length === 0) {
...
function main() {
    ...
    for (const agent of REQUIRED_AGENTS) {
        ...
        if (!entry.last_verified) {
            errors.push(`entries.${agent}.last_verified missing (schema v2 requires it)`);
            continue;
        }
