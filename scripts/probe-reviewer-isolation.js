...
const EXCLUSIONS = JSON.parse(
	fs.readFileSync(
		path.join(ROOT, "reviewer-configs", "peer-exclusions.json"),
		"utf8",
	),
);
const REVIEWER_MCP_JSON = path.join(
	ROOT,
	"reviewer-configs",
	"reviewer-minimal.mcp.json",
);
...
function runCodexContained() {
	...
	if (skipped.length) {
		console.log(
			`[probe:codex] unconfigured mcp_servers (skip): ${skipped.join(", ")}`,
		);
	}
	...
	const appsDisableArgs = (EXCLUSIONS.codex_apps_disable || []).flatMap(
		(id) => ["-c", `apps.${id}.enabled=false`],
	);
	...
	const approveArgs = (EXCLUSIONS.codex_approve_tools || []).flatMap(
		({ server, tool }) => [
			"-c",
...
function classify(result) {
	...
	const memorySucceeded = /mcp:\s*memory\/search_nodes\s*\(completed\)/i.test(
		combined,
	);
	const memoryFailed = /mcp:\s*memory\/search_nodes\s*\(failed\)/i.test(
		combined,
	);
	const memoryStarted = /mcp:\s*memory\/search_nodes\s*started/i.test(combined);
	const autoDenied = /user cancelled MCP tool call|permission denied/i.test(
		combined,
	);
...
function main() {
	...
	const isFailure = (v) =>
		v.startsWith("CONTAINED_MODE_FAILED") ||
		v.startsWith("CONTAINED_MODE_INCOMPLETE");
